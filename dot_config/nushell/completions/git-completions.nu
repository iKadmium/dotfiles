...
module git-completion-utils {
  ...
  # Get remote branches without the remote prefix (e.g., 'feature-branch' instead of 'origin/feature-branch')
  # This is useful for `git switch` which creates local tracking branches
  export def get-remote-branches-for-switch []: nothing -> list<record<value: string, description: string>> {
    get-all-git-remote-refs 
    | each {|x| 
        let parts = ($x.ref | split row '/' -n 2)
        let remote_name = $parts.0
        let branch_name = $parts.1
        {value: $branch_name description: $'Branch, Remote ($remote_name), ($x.obj) ($x.subject)'}
      } 
    | insert style 'blue_italic'
  }
}
...
def "nu-complete git switch" [] {
  use git-completion-utils *
  let local = get-all-git-local-refs | each {|x| {value: $x.ref description: $'Branch, Local, ($x.obj) ($x.subject), (if ($x.upstream | is-not-empty) { $x.upstream } else { "no upstream" } )'} } | insert style 'light_blue'
  let remote = get-remote-branches-for-switch
  {
    options: {
        case_sensitive: false,
        completion_algorithm: prefix,
